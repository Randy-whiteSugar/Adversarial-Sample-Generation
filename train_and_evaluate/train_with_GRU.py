...
import os
from GRU_VAE.gru_vae import rnn_vae, generate_adversarial_examples_pairwise
from GRU_VAE.imdb_data import get_data_with_EOS_token, get_data_without_token, get_data_with_START_token
...
def train_with_GRU():
    ...
    checkpoint_save_path = "./weights/D0_GRU.ckpt"
    if os.path.exists(checkpoint_save_path + '.index'):
        D_0.load_weights(checkpoint_save_path)
        D_1.load_weights("./weights/D1_GRU.ckpt")
        print("已加载判别器参数")
    else:
        print("判别器未训练")
        return

    checkpoint_save_path = "D:/Pycharm/tensorlayer/AdversialSampleGen/GRU_VAE/weights/rnn_vae_gru_3.ckpt"
    if os.path.exists(checkpoint_save_path + '.index'):
        G.load_weights(checkpoint_save_path)
        print("已加载生成器参数")
    else:
        print("生成器未训练")
        return
    ...
    for input_label_0, input_label_1 in tf.data.Dataset.zip((ds_label_0, ds_label_1)):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            joint_loss = total_disc_loss + 10 * vae_loss + 10 * adv_loss
    ...
    D_0_model_name = './weights/D0_GRU.ckpt'
    D_0.save_weights(D_0_model_name)

    D_1_model_name = './weights/D1_GRU.ckpt'
    D_1.save_weights(D_1_model_name)

    G_model_name = './weights/GRU.ckpt'
